...
| Coaching dashboard | Scaffolded | Goal summary and recommendation cards establish the data inputs | `src/experiences/coaching-dashboard/` | Gallery shows goal, recommended, active, and completed sections. |
| Activity lifecycle | Scaffolded | Recommendations have stable IDs | `src/domain/activities.ts` plus dedicated activity MCP tools | Start, complete, and request-alternative actions survive a later `get_goal` call. |
| Durable goal state | Scaffolded | `save_goal` and `get_goal` isolate state access | `src/storage/goal-store.ts` | Goal and activity state survive an MCP server restart and remain user-scoped. |
